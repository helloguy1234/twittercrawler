...
public class ScrapPostThread extends Thread{
    ...
    public void findNumberOfFollower() throws DataFormatException{
    ...
    private float numberStringToFloat (String a) throws DataFormatException{
        ...
        if('0' <= heSoDangChu && heSoDangChu <= '9') return Float.parseFloat(a);
    
        a = a.replace(heSoDangChu,what);
        
        if(heSoDangChu == 'B') return Float.parseFloat(a) * 1000000000;
        if(heSoDangChu == 'M') return Float.parseFloat(a) * 1000000;
        if(heSoDangChu == 'K') return Float.parseFloat(a) * 1000;
    
        return Float.parseFloat(a);
    }    

    public void findRepostedUserId(){
        ...
        if ( userId.equals(postCreatorIdElement.text() ) ) return;
        if ( repostedUserIdList.contains(postCreatorIdElement.text()) ) return;
        
        repostedUserIdList.add(postCreatorIdElement.text());
    }

    public void findCommentedUserId(){
        ...
        for(Element commentedUserElement : listOfCommentedUserElements){
            ...
            //output
            if ( userId.equals(commentedUser_Id.toString() ) ) continue;
            if ( commentedUserIdList.contains(commentedUser_Id.toString() ) ) continue;  
